...
import { loadSavedGame, saveGame, type SavedPlayer } from '../storage/gameSave';
...
export default function GameScreen() {
  ...
  const changeValue = (playerId: number, amount: number) => commit((state) => updatePlayer(state, playerId, (player) => ({ ...player, value: player.value + amount })));
  const removeCounter = (playerId: number, counterId: string) => commit((state) => updatePlayer(state, playerId, (player) => ({ ...player, counters: player.counters.filter((counter) => counter.id !== counterId) })));
  const changeCounter = (playerId: number, counterId: string, field: 'value' | 'secondaryValue', amount: number) => commit((state) => updatePlayer(state, playerId, (player) => ({
    ...player,
  ...
  const toggleCounter = (playerId: number, counterId: string) => commit((state) => updatePlayer(state, playerId, (player) => ({ ...player, counters: player.counters.map((counter) => counter.id === counterId ? { ...counter, active: !counter.active } : counter) })));
  const changeMana = (playerId: number, color: ManaColor, amount: number) => commit((state) => updatePlayer(state, playerId, (player) => ({ ...player, mana: { ...player.mana, [color]: Math.max(0, player.mana[color] + amount) } })));
  const toggleManaColor = (playerId: number, color: ManaColor) => commit((state) => updatePlayer(state, playerId, (player) => ({ ...player, manaColors: player.manaColors.includes(color) ? player.manaColors.filter((item) => item !== color) : [...player.manaColors, color] })));
  ...
  const openRename = (player: Player) => {
    setNamePlayerId(player.id);
  ...
  const resetGame = () => commit(() => cloneState(initialState));
    ...
    <SafeAreaView style={styles.safeArea}>
      ...
      <View style={styles.turnControls}>
        <ScrollView horizontal showsHorizontalScrollIndicator={false} contentContainerStyle={styles.phaseTrack}>
          {rules.phases.map((phase, index) => <Pressable key={phase} onPress={() => selectPhase(index)} style={[styles.phaseChip, index === game.activePhase && styles.activePhaseChip]}><Text style={[styles.phaseText, index === game.activePhase && styles.activePhaseText]}>{phase}</Text></Pressable>)}
        </ScrollView>
        <Pressable onPress={advance} style={styles.nextButton}><Text style={styles.nextButtonText}>{isLastPhase ? 'END TURN' : 'NEXT PHASE'}</Text><Text style={styles.nextButtonHint}>{isLastPhase ? `PASS TO ${game.players[(game.activePlayer + 1) % game.players.length].name}` : rules.phases[game.activePhase + 1]}</Text></Pressable>
      </View>

      <Modal transparent visible={namePlayerId !== null} animationType="fade" onRequestClose={() => setNamePlayerId(null)}>
        <View style={styles.modalBackdrop}><View style={styles.nameCard}>
          <Text style={styles.modalTitle}>Rename player</Text>
          <TextInput autoFocus value={nameDraft} onChangeText={setNameDraft} onSubmitEditing={saveName} maxLength={18} selectTextOnFocus style={styles.nameInput} placeholder="Player name" placeholderTextColor="#666D7D" />
          <View style={styles.modalActions}><Pressable onPress={() => setNamePlayerId(null)} style={styles.cancelButton}><Text style={styles.cancelText}>CANCEL</Text></Pressable><Pressable onPress={saveName} style={styles.addButton}><Text style={styles.addText}>SAVE NAME</Text></Pressable></View>
        </View></View>
      </Modal>
...
function PlayerPanel({ player, metric, largeStep, active, labels, supportsMana, manaOpen, onRename, onToggleMana, onManaColors, onChangeMana, onChangeValue, onChangeCounter, onToggleCounter, onRemoveCounter, onAddCounter }: {
  ...
  return <View style={[styles.panel, active && styles.activePanel]}>
    ...
    <Text style={styles.metric}>{metric}</Text><Text style={styles.value}>{player.value}</Text>
...
const styles = StyleSheet.create({
  ...
  topBar: { height: 64, flexDirection: 'row', alignItems: 'center', justifyContent: 'space-between', gap: 7 },
  topButton: { minWidth: 72, paddingVertical: 9, paddingHorizontal: 9, borderRadius: 11, backgroundColor: '#151820', alignItems: 'center' },
  disabledButton: { opacity: 0.35 },
  topButtonText: { color: '#AEB3C1', fontSize: 9, fontWeight: '900' },
  turnStatus: { flex: 1, alignItems: 'center' }, gameMode: { color: '#8F7CFF', fontSize: 7, fontWeight: '900' }, turnLabel: { color: '#676D7D', fontSize: 7, fontWeight: '900' }, turnName: { color: '#F4F3FF', fontSize: 15, fontWeight: '900' },
  boardScroll: { flex: 1, minHeight: 0 }, grid: { flexDirection: 'row', flexWrap: 'wrap', alignItems: 'flex-start', gap: 8, paddingBottom: 18 },
  panel: { width: '48.5%', minHeight: 230, borderRadius: 18, borderWidth: 1, borderColor: '#252936', backgroundColor: '#11141B', alignItems: 'center', padding: 8, paddingTop: 16 },
  activePanel: { borderColor: '#8F7CFF', borderWidth: 3, backgroundColor: '#181528' }, activeBadge: { position: 'absolute', top: 7, right: 9, color: '#B8ACFF', fontSize: 7, fontWeight: '900' },
  playerName: { color: '#AEB3C1', fontSize: 10, fontWeight: '900' }, metric: { color: '#5F6573', fontSize: 7, fontWeight: '900' }, value: { color: '#FFFFFF', fontSize: 43, lineHeight: 46, fontWeight: '900' },
  controls: { flexDirection: 'row', gap: 5 }, counterButton: { minWidth: 42, paddingVertical: 7, borderRadius: 9, backgroundColor: '#222630', alignItems: 'center' }, counterButtonText: { color: '#E5E1FF', fontSize: 12, fontWeight: '900' },
  manaRail: { position: 'absolute', left: 6, top: 8, zIndex: 4, alignItems: 'flex-start' }, manaButton: { width: 46, minHeight: 42, borderRadius: 10, backgroundColor: '#27213F', borderWidth: 1, borderColor: '#6655B8', alignItems: 'center', justifyContent: 'center' }, manaTitle: { color: '#BDB6FF', fontSize: 6, fontWeight: '900' }, manaTotal: { color: '#FFFFFF', fontSize: 16, fontWeight: '900' }, manaDrawer: { marginTop: 5, width: 122, borderRadius: 12, backgroundColor: '#171A22', borderWidth: 1, borderColor: '#343948', padding: 7, gap: 5 }, chooseColors: { color: '#BDB6FF', fontSize: 7, fontWeight: '900', textAlign: 'center', paddingVertical: 5 }, emptyText: { color: '#757B8A', fontSize: 8, textAlign: 'center', paddingVertical: 6 }, manaRow: { flexDirection: 'row', alignItems: 'center', justifyContent: 'space-between' }, manaSymbol: { color: '#FFFFFF', fontSize: 12, fontWeight: '900' },
  playerCounters: { width: '100%', flexDirection: 'row', flexWrap: 'wrap', gap: 6, paddingTop: 9, justifyContent: 'center' }, trackedCounter: { minWidth: 96, minHeight: 48, borderRadius: 10, backgroundColor: '#1B1E27', paddingHorizontal: 8, paddingVertical: 6, justifyContent: 'center' }, statsCounter: { minWidth: 145 }, trackedLabel: { color: '#9DA3B2', fontSize: 7, fontWeight: '900', textAlign: 'center', marginBottom: 2 }, valueControl: { flexDirection: 'row', justifyContent: 'space-around', alignItems: 'center' }, smallControl: { color: '#BDB6FF', fontSize: 17, fontWeight: '900', paddingHorizontal: 5 }, trackedValue: { color: '#FFFFFF', fontSize: 14, fontWeight: '900' }, statLabel: { color: '#777D8D', fontSize: 7, fontWeight: '900' }, toggleButton: { borderRadius: 8, backgroundColor: '#242833', paddingVertical: 7, paddingHorizontal: 10, alignItems: 'center' }, toggleActive: { backgroundColor: '#4D3FA3' }, toggleText: { color: '#FFFFFF', fontSize: 8, fontWeight: '900' }, deleteCounter: { position: 'absolute', right: -5, top: -7, width: 22, height: 22, borderRadius: 11, backgroundColor: '#FF5F6D', alignItems: 'center', justifyContent: 'center' }, deleteText: { color: '#FFFFFF', fontSize: 17, fontWeight: '900' }, addCounterChip: { minWidth: 96, height: 48, borderRadius: 10, borderWidth: 1, borderStyle: 'dashed', borderColor: '#4C5060', paddingHorizontal: 12, alignItems: 'center', justifyContent: 'center' }, addCounterText: { color: '#8F7CFF', fontSize: 8, fontWeight: '900' },
