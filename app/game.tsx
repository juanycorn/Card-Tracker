...
type CounterKind = 'single' | 'stats' | 'toggle' | 'mana';
type CounterRole =
  | 'mana'
  | 'enemy'
  | 'treasure'
...
const THEMES: Record<string, Theme> = {
  arcane: {
    name: 'Arcane',
    labels: {
      mana: 'Mana Pool', enemy: 'Creature Modifier', treasure: 'Treasure', food: 'Food', resource: 'Resource', buff: 'Buff', debuff: 'Debuff', objective: 'Objective', poison: 'Poison', energy: 'Energy', experience: 'Experience', storm: 'Storm Count', monarch: 'Monarch', initiative: 'Initiative', daynight: 'Day / Night', custom: 'Counter',
    },
  },
  fantasy: {
    name: 'Fantasy Raid',
    labels: {
      mana: 'Magic Pool', enemy: 'Enemy Modifier', treasure: 'Gold', food: 'Rations', resource: 'Supplies', buff: 'Blessing', debuff: 'Curse', objective: 'Quest', poison: 'Venom', energy: 'Stamina', experience: 'Renown', storm: 'Combo', monarch: 'Crowned', initiative: 'Dungeon Lead', daynight: 'Day / Night', custom: 'Counter',
    },
  },
  scifi: {
    name: 'Sci-Fi',
    labels: {
      mana: 'Power Pool', enemy: 'Target Modifier', treasure: 'Credits', food: 'Med Packs', resource: 'Energy Cells', buff: 'Upgrade', debuff: 'Malfunction', objective: 'Mission', poison: 'Contamination', energy: 'Charge', experience: 'Intel', storm: 'Chain', monarch: 'Command', initiative: 'Priority', daynight: 'Cycle', custom: 'Counter',
    },
  },
};
...
const COUNTER_KIND: Record<CounterRole, CounterKind> = {
  mana: 'mana', enemy: 'stats', treasure: 'single', food: 'single', resource: 'single', buff: 'single', debuff: 'single', objective: 'single', poison: 'single', energy: 'single', experience: 'single', storm: 'single', monarch: 'toggle', initiative: 'toggle', daynight: 'toggle', custom: 'single',
};

const COUNTER_GROUPS: { title: string; roles: CounterRole[] }[] = [
  { title: 'MANA', roles: ['mana'] },
...
export default function GameScreen() {
  ...
  const isMagic = gameName.toUpperCase() === 'MAGIC';
  ...
  const [counterRole, setCounterRole] = useState<CounterRole>(isMagic ? 'mana' : 'resource');
  ...
  const changeMana = (playerId: number, counterId: string, color: ManaColor, amount: number) => updatePlayers((player) => player.id === playerId ? {
    ...player,
    counters: player.counters.map((counter) => counter.id === counterId ? { ...counter, mana: { ...(counter.mana ?? EMPTY_MANA), [color]: Math.max(0, (counter.mana?.[color] ?? 0) + amount) } } : counter),
  } : player);

  const clearManaForPlayer = (playerIndex: number) => {
    const playerId = players[playerIndex]?.id;
    if (!playerId) return;
    updatePlayers((player) => player.id === playerId ? { ...player, counters: player.counters.map((counter) => counter.kind === 'mana' ? { ...counter, mana: { ...EMPTY_MANA } } : counter) } : player);
  };
  ...
  const selectPhase = (index: number) => {
    if (index !== activePhase && isMagic) clearManaForPlayer(activePlayer);
    ...
    <SafeAreaView style={styles.safeArea}>
      ...
      <ScrollView style={styles.boardScroll} contentContainerStyle={styles.grid} showsVerticalScrollIndicator nestedScrollEnabled persistentScrollbar>
        {players.map((player, index) => (
          <PlayerPanel key={player.id} player={player} metric={metric} largeStep={largeStep} isActive={index === activePlayer} theme={theme} onChange={changeValue} onChangeSingle={changeSingle} onChangeStat={changeStat} onToggle={toggleCounter} onChangeMana={changeMana} onRemoveCounter={removeCounter} onAddCounter={() => setCounterPlayerId(player.id)} />
        ))}
      </ScrollView>
      ...
      <Modal transparent visible={counterPlayerId !== null} animationType="fade" onRequestClose={() => setCounterPlayerId(null)}>
        <View style={styles.modalBackdrop}><View style={styles.modalCard}>
          <Text style={styles.modalTitle}>Add typed counter</Text><Text style={styles.modalSubtitle}>Scroll for more. Mana tracks white, blue, black, red, green, and colorless separately.</Text>
...
function PlayerPanel({ player, metric, largeStep, isActive, theme, onChange, onChangeSingle, onChangeStat, onToggle, onChangeMana, onRemoveCounter, onAddCounter }: {
  player: Player; metric: string; largeStep: number; isActive: boolean; theme: Theme;
  ...
  onChangeMana: (playerId: number, counterId: string, color: ManaColor, amount: number) => void;
  onRemoveCounter: (playerId: number, counterId: string) => void; onAddCounter: () => void;
}) {
  ...
  return <View style={[styles.panel, isActive && styles.activePanel]}>
    ...
    <ScrollView horizontal showsHorizontalScrollIndicator={false} contentContainerStyle={styles.playerCounters}>
      {player.counters.map((counter) => <Pressable key={counter.id} onLongPress={() => setDeleteMode(counter.id)} style={[styles.trackedCounter, counter.kind === 'stats' && styles.statsCounter, counter.kind === 'mana' && styles.manaCounter]}>
        <Text style={styles.trackedLabel}>{theme.labels[counter.role]}{counter.temporary ? ' · EOT' : ''}</Text>
        {counter.kind === 'single' && <View style={styles.trackedControls}><Pressable onPress={() => onChangeSingle(player.id, counter.id, -1)}><Text style={styles.smallControl}>−</Text></Pressable><Text style={styles.trackedValue}>{counter.value}</Text><Pressable onPress={() => onChangeSingle(player.id, counter.id, 1)}><Text style={styles.smallControl}>+</Text></Pressable></View>}
        {counter.kind === 'stats' && <View style={styles.statRows}><StatControl label="PWR" value={counter.value} onMinus={() => onChangeStat(player.id, counter.id, 'value', -1)} onPlus={() => onChangeStat(player.id, counter.id, 'value', 1)} /><StatControl label="TGH" value={counter.secondaryValue ?? 0} onMinus={() => onChangeStat(player.id, counter.id, 'secondaryValue', -1)} onPlus={() => onChangeStat(player.id, counter.id, 'secondaryValue', 1)} /></View>}
        {counter.kind === 'toggle' && <Pressable onPress={() => onToggle(player.id, counter.id)} style={[styles.toggleButton, counter.active && styles.toggleButtonActive]}><Text style={[styles.toggleText, counter.active && styles.toggleTextActive]}>{counter.active ? 'ACTIVE' : 'INACTIVE'}</Text></Pressable>}
        {counter.kind === 'mana' && <View style={styles.manaGrid}>{MANA_COLORS.map((color) => <View key={color} style={styles.manaCell}><Text style={[styles.manaSymbol, styles[`mana${color}` as keyof typeof styles] as object]}>{color}</Text><View style={styles.manaControls}><Pressable onPress={() => onChangeMana(player.id, counter.id, color, -1)}><Text style={styles.manaButton}>−</Text></Pressable><Text style={styles.manaValue}>{counter.mana?.[color] ?? 0}</Text><Pressable onPress={() => onChangeMana(player.id, counter.id, color, 1)}><Text style={styles.manaButton}>+</Text></Pressable></View></View>)}</View>}
