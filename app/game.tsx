...
type CounterRole =
  ...
  | 'monarch' | 'initiative' | 'daynight' | 'custom';
...
const THEMES: Record<string, Theme> = {
  arcane: { name: 'Arcane', labels: { enemy: 'Creature Modifier', treasure: 'Treasure', food: 'Food', resource: 'Resource', buff: 'Buff', debuff: 'Debuff', objective: 'Objective', poison: 'Poison', energy: 'Energy', experience: 'Experience', storm: 'Storm Count', monarch: 'Monarch', initiative: 'Initiative', daynight: 'Day / Night', custom: 'Counter' } },
  fantasy: { name: 'Fantasy Raid', labels: { enemy: 'Enemy Modifier', treasure: 'Gold', food: 'Rations', resource: 'Supplies', buff: 'Blessing', debuff: 'Curse', objective: 'Quest', poison: 'Venom', energy: 'Stamina', experience: 'Renown', storm: 'Combo', monarch: 'Crowned', initiative: 'Dungeon Lead', daynight: 'Day / Night', custom: 'Counter' } },
  scifi: { name: 'Sci-Fi', labels: { enemy: 'Target Modifier', treasure: 'Credits', food: 'Med Packs', resource: 'Energy Cells', buff: 'Upgrade', debuff: 'Malfunction', objective: 'Mission', poison: 'Contamination', energy: 'Charge', experience: 'Intel', storm: 'Chain', monarch: 'Command', initiative: 'Priority', daynight: 'Cycle', custom: 'Counter' } },
};
...
const COUNTER_KIND: Record<CounterRole, CounterKind> = {
  enemy: 'stats', treasure: 'single', food: 'single', resource: 'single', buff: 'single', debuff: 'single', objective: 'single', poison: 'single', energy: 'single', experience: 'single', storm: 'single', monarch: 'toggle', initiative: 'toggle', daynight: 'toggle', custom: 'single',
};
...
export default function GameScreen() {
  ...
  const clearManaForPlayer = (playerIndex: number) => {
    ...
    if (!playerId) return;
    updatePlayers((player) => player.id === playerId ? { ...player, mana: { ...EMPTY_MANA } } : player);
  };
    ...
    <SafeAreaView style={styles.safeArea}>
      ...
      <ScrollView style={styles.boardScroll} contentContainerStyle={styles.grid} showsVerticalScrollIndicator nestedScrollEnabled persistentScrollbar>
        {players.map((player, index) => (
          <PlayerPanel
            ...
            onOpenManaColors={() => setManaColorPickerId(player.id)}
      ...
      <Modal transparent visible={counterPlayerId !== null} animationType="fade" onRequestClose={() => setCounterPlayerId(null)}>
        <View style={styles.modalBackdrop}><View style={styles.modalCard}>
          <Text style={styles.modalTitle}>Add typed counter</Text><Text style={styles.modalSubtitle}>Scroll for more. Counters grow the player panel vertically.</Text>
      ...
      <Modal transparent visible={manaColorPickerId !== null} animationType="fade" onRequestClose={() => setManaColorPickerId(null)}>
        <View style={styles.modalBackdrop}><View style={styles.manaPickerCard}>
          ...
          <Text style={styles.modalSubtitle}>Only selected colors will appear in this player's mana drawer.</Text>
          <View style={styles.manaPickerGrid}>
            {MANA_COLORS.map((color) => {
              const player = players.find((item) => item.id === manaColorPickerId);
              const selected = player?.manaColors.includes(color) ?? false;
              return <Pressable key={color} onPress={() => manaColorPickerId && toggleManaColor(manaColorPickerId, color)} style={[styles.manaPickerButton, selected && styles.selectedManaColor]}><Text style={[styles.manaSymbol, styles[`mana${color}` as keyof typeof styles] as object]}>{color}</Text><Text style={styles.roleName}>{selected ? 'SHOWN' : 'HIDDEN'}</Text></Pressable>;
            })}
          </View>
...
function PlayerPanel({ player, metric, largeStep, isActive, theme, showMana, manaOpen, onToggleMana, onOpenManaColors, onChangeMana, onChange, onChangeSingle, onChangeStat, onToggle, onRemoveCounter, onAddCounter }: {
  ...
  return <View style={[styles.panel, isActive && styles.activePanel]}>
    ...
    <View style={styles.playerCounters}>
      {player.counters.map((counter) => <Pressable key={counter.id} onLongPress={() => setDeleteMode(counter.id)} style={[styles.trackedCounter, counter.kind === 'stats' && styles.statsCounter]}>
