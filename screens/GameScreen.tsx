...
function PlayerPanel({ player, theme, metric, largeStep, active, supportsMana, manaOpen, onRename, onToggleMana, onManaColors, onChangeMana, onChangeValue, onChangeCounter, onToggleCounter, onRemoveCounter, onAddCounter }: {
  ...
  return <Animated.View style={[styles.panel, { transform: [{ scale: panelScale }], borderColor, backgroundColor }]}> 
    ...
    <View style={styles.controls}>{[-largeStep, -1, 1, largeStep].map((amount, index) => <Pressable key={`${amount}-${index}`} onPress={() => onChangeValue(player.id, amount)} style={({ pressed }) => [styles.counterButton, { borderColor: theme.colors.border }, pressed && styles.pressedButton]}><Text style={styles.counterButtonText}>{amount > 0 ? `+${amount}` : amount}</Text></Pressable>)}</View>
