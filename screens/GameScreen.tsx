...
function PlayerPanel({ player, metric, largeStep, active, labels, supportsMana, manaOpen, onRename, onToggleMana, onManaColors, onChangeMana, onChangeValue, onChangeCounter, onToggleCounter, onRemoveCounter, onAddCounter }: {
  ...
  useEffect(() => {
    Animated.spring(drawer, { toValue: manaOpen ? 1 : 0, friction: 7, tension: 110, useNativeDriver: true }).start();
  }, [drawer, manaOpen]);

  const manaTotal = Object.values(player.mana).reduce((sum, value) => sum + value, 0);
  const borderColor = glow.interpolate({ inputRange: [0, 1], outputRange: ['#252936', '#A899FF'] });
  const backgroundColor = glow.interpolate({ inputRange: [0, 1], outputRange: ['#11141B', '#211A38'] });
...
function AnimatedCounter({ counter, playerId, label, deleteId, setDeleteId, onChangeCounter, onToggleCounter, onRemoveCounter }: {
  counter: TrackedCounter; playerId: number; label: string; deleteId: string | null; setDeleteId: (id: string | null) => void;
  ...
  return <Animated.View style={{ opacity: appear, transform: [{ scale: appear }, { translateY: appear.interpolate({ inputRange: [0, 1], outputRange: [12, 0] }) }] }}><Pressable onLongPress={() => setDeleteId(counter.id)} style={[styles.trackedCounter, kind === 'stats' && styles.statsCounter]}><Text style={styles.trackedLabel}>{label}{counter.temporary ? ' · EOT' : ''}</Text>{kind === 'single' && <ValueControl value={counter.value} onMinus={() => onChangeCounter(playerId, counter.id, 'value', -1)} onPlus={() => onChangeCounter(playerId, counter.id, 'value', 1)} />}{kind === 'stats' && <><ValueControl label="A" value={counter.value} onMinus={() => onChangeCounter(playerId, counter.id, 'value', -1)} onPlus={() => onChangeCounter(playerId, counter.id, 'value', 1)} /><ValueControl label="B" value={counter.secondaryValue ?? 0} onMinus={() => onChangeCounter(playerId, counter.id, 'secondaryValue', -1)} onPlus={() => onChangeCounter(playerId, counter.id, 'secondaryValue', 1)} /></>}{kind === 'toggle' && <Pressable onPress={() => onToggleCounter(playerId, counter.id)} style={[styles.toggleButton, counter.active && styles.toggleActive]}><Text style={styles.toggleText}>{counter.active ? 'ACTIVE' : 'INACTIVE'}</Text></Pressable>}{deleteId === counter.id && <Pressable onPress={() => onRemoveCounter(playerId, counter.id)} style={styles.deleteCounter}><Text style={styles.deleteText}>×</Text></Pressable>}</Pressable></Animated.View>;
}
...
const styles = StyleSheet.create({
  safeArea: { flex: 1, backgroundColor: '#080A0F', paddingHorizontal: 12, paddingBottom: 8 }, topBar: { height: 64, flexDirection: 'row', alignItems: 'center', justifyContent: 'space-between', gap: 7 }, topButton: { minWidth: 72, paddingVertical: 9, paddingHorizontal: 9, borderRadius: 11, backgroundColor: '#151820', alignItems: 'center' }, disabledButton: { opacity: 0.35 }, topButtonText: { color: '#AEB3C1', fontSize: 9, fontWeight: '900' }, turnStatus: { flex: 1, alignItems: 'center' }, gameMode: { color: '#8F7CFF', fontSize: 7, fontWeight: '900' }, turnLabel: { color: '#676D7D', fontSize: 7, fontWeight: '900' }, turnName: { color: '#F4F3FF', fontSize: 15, fontWeight: '900' },
  boardScroll: { flex: 1, minHeight: 0 }, grid: { flexDirection: 'row', flexWrap: 'wrap', alignItems: 'flex-start', gap: 8, paddingBottom: 18 }, panel: { width: '48.5%', minHeight: 230, borderRadius: 18, borderWidth: 2, alignItems: 'center', padding: 8, paddingTop: 16, elevation: 5 }, activeBadge: { position: 'absolute', top: 7, right: 9, color: '#D5CEFF', fontSize: 7, fontWeight: '900' }, playerName: { color: '#AEB3C1', fontSize: 10, fontWeight: '900' }, metric: { color: '#5F6573', fontSize: 7, fontWeight: '900' }, value: { color: '#FFFFFF', fontSize: 43, lineHeight: 46, fontWeight: '900' }, controls: { flexDirection: 'row', gap: 5 }, counterButton: { minWidth: 42, paddingVertical: 7, borderRadius: 9, backgroundColor: '#222630', alignItems: 'center' }, pressedButton: { transform: [{ scale: 0.9 }], opacity: 0.75 }, counterButtonText: { color: '#E5E1FF', fontSize: 12, fontWeight: '900' },
